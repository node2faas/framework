...
function checkDepedencies(){
  ...
  if (!global.fs.existsSync(`${__dirname}/../terraform/gcp.json`)) {
    global.fs.writeFileSync(`${__dirname}/../terraform/gcp.json`, '');
  }
...
function getProvider() {
  if (global.provider == 'clean' && global.fs.existsSync(global.homedir+'/.node2faas/provider')){
	...
	if (global.fs.existsSync(global.homedir+'/.node2faas/provider')){
		let file = fs.readFileSync(global.homedir+'/.node2faas/provider');
    console.log('Initializing...');
	fs.mkdirSync(global.orchestratorFolder,{ recursive: true });
	...
	return new Promise((resolve, reject) => {
		...
		rl_key.question('  Provider: ', (provider_id) => {
		   	switch(provider_id) {
				...
				let stream = global.fs.createWriteStream(global.homedir+'/.node2faas/provider');
