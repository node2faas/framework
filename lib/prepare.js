...
function checkDepedencies(){
  ...
  if (!checkTerraform){
    if (global.shelljs.exec('test -f ~/terraform/terraform; echo $?',{silent: true}).stdout=='1\n'){
      global.shelljs.exec('npm install terraform-latest');
      global.shelljs.exec('source ~/.env');
    }
    global.terraform = '~/terraform/terraform';
  } else {
...
function getProvider() {
  if (global.provider == 'clean' && global.fs.existsSync(global.homedir+'/.node2faas_provider')){
    ...
    global.shelljs.exec('cd ~/node_modules/node2faas/terraform ; '+global.terraform+' init',{silent:!global.debug});
