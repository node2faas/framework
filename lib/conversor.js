    ...
    global.fs.readdirSync(target).forEach(file => {
      if (file !== 'node_modules'){
        ...
        if (fs.lstatSync(target+'/'+file).isDirectory()){
          this.convert(target+'/'+file, outputFolder);
        } else {
          conversion('/'+file, target, outputFolder);
        }
      }
		})
}
 ...
 * @param {string}   outputFolder The output folder where file will be generated.
 */
function conversion(file, folder, outputFolder){
  outputFolder = outputFolder.replace(/\/\//g, '/').replace(/\/\//g, '/')
  const outputSub = folder.replace(/\/\//g, '/').replace(/\/\//g, '/').replace(/\.\.\//g, '').replace(outputFolder.replace('output/', ''),"")
  switch(file.split('.')[1]) {
	    case 'js':
        if (!fs.existsSync(`${outputFolder}${outputSub}${file}`)){
  				try {
            global.fs.mkdirSync(outputFolder + outputSub, {recursive:true})
            const source = (folder+file).replace(/\/\//g, '/').replace(/\/\//g, '/')
            const destination = outputFolder + outputSub + file
            global.fs.copyFileSync(source, destination);
            global.common.debug('File '+folder+file+' was copied to '+outputFolder+outputSub+file);
          } catch(err) {
            global.common.debug(err);
          }
        }
        loadRequires(file, folder);
				let functions = extractFunctions(file, folder, outputFolder);
        functions.forEach(function(func,index,array){
          global.common.debug('Creating zipfile for function '+func.name);
  				let zipfile = createZipFile(func.name, normalizeCode(func.name, func.algoritm, func.parameters),`${folder}`);
          ...
          if (global.method=='apply') {
            ...
            replaceHashToNewFunctionCode(outputFolder+outputSub+file,func.name,new_code)
  					global.common.debug('File '+outputFolder+outputSub+file+' was updated with new code for function '+func.name);
          } else {
      ...
      default:
        if (global.method == 'apply'){
          if (!fs.lstatSync(folder+file).isDirectory()){
            try {
              fs.copyFileSync(folder+file, outputFolder+outputSub+file);
              global.common.debug('File (untouched - just copied to output): '+outputFolder+outputSub+file);
          	} catch(err) {
          	  global.common.debug(err);
          	}
          }
...
function createZipFile(functionName, content, targetFolder){
  ...
  let zipfile = `${filePath}${filename}.zip`;
...
function normalizeCode(functionName, code, parameters){
  ...
  switch(global.provider) {
    ...
    case "aws":
      ...
      //let splitTerm = 'event.queryStringParameters';
      let strParams = ''
...
function extractFunctions(file, folder, outputFolder){
  let analyzer = require('./analyzer')
	...
	let qtd_exports = global.shelljs.exec("cat "+folder+file+"    | \
	                                      grep ^exports.   | \
																				wc -l            | \
																				sed -e 's/ //g'  ",
																				{silent:true})
																				.stdout;
  let file_content = global.shelljs.exec("cat "+folder+file,{silent:true}).stdout;
  file_content = file_content.replace(new RegExp(/\nexports[.]/, 'g'),'\nfaasificable.');
   ...
   if (func.name && !global.funcList[func.name]){
       ...
       global.common.debug('Function '+func.name+' wasn\'t selected to be published on FaaS');
...
function assemblyFunction(functionName, parameters, uri){
  ...
  switch(global.provider) {
    case "aws":
      var returnString =  "    if (typeof res !== 'undefined') { \
      ...
      break;
    case "gcp":
      var returnString = "res.json(axiosRes.data);"
      break;
    case "azure":
      preStr="";
      var returnString = "res.json(axiosRes.data);"
...
function publication(functionName, codePath){
    ...
    if (!fs.existsSync(funcFolder)){
  		fs.mkdirSync(funcFolder,{ recursive: true });
  	}
    if (fs.existsSync(funcFolder+'/terraform.tfstate')){
      funcPublicated = true;
  	  global.shelljs.exec(`mv ${funcFolder}/*tfstate* terraform`);
    }
    ...
    if (method!='destroy' && funcPublicated){
      ...
      global.shelljs.exec('cd terraform ;'+
    ...
    result = global.shelljs.exec('cd terraform ;'+
                            global.terraform+' '+global.method+' -auto-approve '+
                            providerParams + generalParams, {silent:!global.debug});
    result = result.stdout+result.stderr
    
    if (fs.existsSync('terraform/terraform.tfstate')){
      global.shelljs.exec('mv terraform/*tfstate* '+funcFolder);
    }
