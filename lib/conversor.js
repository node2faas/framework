...
exports.convert = function(target, outputFolder){
    global.fs.readdirSync(target).forEach(file => {
      if (file !== 'node_modules'){
        global.sumary['files']++;
        let filename = global.common.cleanDoubleSlashes(target+'/'+file);
          ...
          if (!fs.lstatSync(source).isDirectory()){
            try {
              ...
              global.common.debug(`File ${source} (untouched - just copied to output): ${destination} `);
          	  ...
          	  global.common.debug(`Error: unable to copy untoched file: ${err}`);
          	}
          }
        }
	}
...
function publication(functionName, codePath){
    ...
    let funcFolder = global.homedir+'/.node2faas/.exec-'+global.provider+'-'+functionName;
    ...
    if (fs.existsSync(`${global.orchestratorFolder}/terraform.tfstate`)){
