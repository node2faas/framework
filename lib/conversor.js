...
function normalizeCode(functionName, code, parameters){
    ...
    case "gcp":
        ...
        for (const i in parameters){
            strParams += `    let ${parameters[i]} = JSON.parse(res.body).${parameters[i]} \n`
        }
        newcode = "\
...
function splitFunction(code){
    ...
    if (temp){
        temp = temp.split("{")[0]
            .replace("(","").replace(")","")
            .replace("function","").replace(" ","").trim().split(",").forEach(function(element){
...
function assemblyFunction(functionName, parameters, uri){
    ...
    case "aws":
        ...
        break
    case "gcp":
        returnString = "res.json(axiosRes.data)"
        break
    case "azure":
        preStr=""
        returnString = "res.json(axiosRes.data)"
