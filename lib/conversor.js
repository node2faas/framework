...
function createZipFile(functionName, content){
  if (!fs.existsSync('tmp/')){
	...
	let file = functionName+".js";
	let zipfile = "tmp/"+functionName+".zip";
...
function publish(functionName, codePath){
		let result;
			switch(global.provider) {
				case 'aws':
				  let aws_access_key_id = getCredentialValue('aws_access_key_id');
					...
					let region = getCredentialValue('region');
          console.log('Publishing function '+functionName+' on AWS...');
					...
					break;
				default:
				  console.log('Error: provider no available');
					process.exit(2);
			}
      checkPublicationErrors(result);
      let data = {'functionName': functionName ,
			            'url': result.split('_url = ')[1]
									             .trim()
									             .replace(' ','')
															 .replace('\n','')
														   .replace('[0m','')};
 ...
 */
function checkPublicationErrors(result){
