...
function conversion(file, folder, outputFolder){
	switch(file.split('.')[1]) {
	    case 'js':
        if (!fs.existsSync(`${outputFolder}${folder}${file}`)){
  				try {
            fs.copyFileSync(folder+file, outputFolder+folder+file);
            global.common.debug('File '+folder+file+' was copied to '+outputFolder+folder+file);
          } catch(err) {
            global.common.debug(err);
          }
        }
        loadRequires(file, folder);
				let functions = extractFunctions(folder+file, outputFolder);
        ...
        let zipfile = createZipFile(func.name, normalizeCode(func.name, func.algoritm, func.parameters),`${folder}`);
          if (global.method=='apply') {
            ...
            replaceHashToNewFunctionCode(outputFolder+folder+file,func.name,new_code)
  ...
  switch(global.provider) {
    ...
    case "azure":
      ...
      zip.addFile(functionName+'/'+filename, Buffer.alloc(content.length, content), "Function "+functionName+" - automatic code with Node2FaaS");
    	zip.addLocalFolder(`${outputFolder}/node_modules`,'node_modules')
      break;
    default:
 ...
 */
function extractFunctions(file, outputFolder){
  ...
  file_content = file_content.replace(new RegExp(/\nexports[.]/, 'g'),'\nfaasificable.');
   ...
   if (func.name && !global.funcList[func.name]){
       ...
       console.log('Function '+func.name+' was elegible to be published on FaaS');
	     functions.push(func);
       global.sumary['funcs_elegibles']++;
     } else {
       global.common.debug('Function '+func.name+' wasn\'t elegible to be published on FaaS');
    ...
    functions.forEach(function(element,index,array){
			replaceFunctionCodeToHash(outputFolder+file,element.name);
	  });
	} else {
