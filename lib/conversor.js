	...
	switch(file.split('.')[1]) {
        ...
        loadRequires(file, folder);
				...
				functions.forEach(function(func,index,array){
					...
					let func_publicated = publish(func.name,zipfile);
...
function normalizeCode(functionName, code, parameters){
	switch(global.provider) {
	 	case "aws","gcp","azure":
			...
			break
	default:
		console.log('Error: Sorry, this provider is not available! (Normalization)');
		process.exit(2);
	}
}
...
function publish(functionName, codePath){
		let result;
			switch(global.provider) {
				case 'aws':
				  let aws_access_key_id = getCredentialValue('aws_access_key_id');
					let aws_secret_access_key = getCredentialValue('aws_secret_access_key');
					var region = getCredentialValue('region');
          console.log('Publishing function '+functionName+' on AWS...');
					result = global.shelljs.exec('cd ~/node_modules/node2faas/terraform ;'+
					                    'terraform apply -auto-approve '+
															...
															'-target=module.node2faas-aws ',
															{silent:!global.debug}).stdout;
					break;
        case 'gcp':
				  let json_file = getCredentialValue('json_file');
          fs.copyFile(json_file, 'gcp.json', (err) => {
  					if (err) throw err;
  					global.common.debug('File '+json_file+' was copied to gcp.json');
  				});
          let json_data = global.fs.readFileSync('gcp.json');
          var project = JSON.parse(json_data).project_id;
          var region = getCredentialValue('region');
          console.log('Publishing function '+functionName+' on Google Cloud Platform...');
					result = global.shelljs.exec('cd ~/node_modules/node2faas/terraform ;'+
					                    'terraform apply -auto-approve '+
															'-var "region='+region+'" '+
                              '-var "project='+project+'" '+
															'-var "sourcecode_zip_path='+global.homedir+'/node_modules/node2faas/'+codePath+'" '+
															'-var "name='+functionName+'" '+
															'-target=module.node2faas-gcp ',
															{silent:!global.debug});
          result = result.stdout+result.stderr
					break;
        case 'azure':
  				  let subscription_id = getCredentialValue('subscription_id');
            let tenant_id = getCredentialValue('tenant_id');
            let client_id = getCredentialValue('client_id');
            let client_secret = getCredentialValue('client_secret');
            var region = getCredentialValue('region');
            console.log('Publishing function '+functionName+' on Microsoft Azure...');
  					result = global.shelljs.exec('cd ~/node_modules/node2faas/terraform ;'+
  					                    'terraform apply -auto-approve '+
  															'-var "region='+region+'" '+
