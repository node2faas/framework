...
function cleanProvidersFiles(){
	...
	global.shelljs.exec('rm -f '+global.homedir+'/.node2faas_provider');
}
...
exports.sumary = function(){
	...
	for (var i in global.funcList){
		common.debug(JSON.stringify(global.funcList[i]));
	}
}
 ...
 */
 exports.addDependencyToOutput = (targetFolder, outputFolder, package, version) => {
	...
	global.fs.mkdirSync(outputFolder+targetFolder.replace(/\.\.\//g, ''),{ recursive: true })
 ...
 */
 exports.downloadDependencies = (folder) => {	
	if (global.fs.existsSync(`${folder}/package.json`)) {
	  	const result = global.shelljs.exec(`cd ${folder} ; \
						   npm install`,
						   {silent:true})
						   .stdout
		return result
